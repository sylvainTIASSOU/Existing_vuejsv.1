...
class ChartManager {
  ...
  initializeChart() {
    ...
    this.chart = createChart(this.container, chartProperties);
  ...
  async loadData(data) {
    try {
      ...
      this.xspan = data
        .map((item) => Math.floor(new Date(item.timestamp).getTime() / 1000))
  ...
  handleChartClick(param) {
    ...
    this.isHovered ? this.startDrag(xTs, yPrice) : this.handleLineDrawing(xTs, yPrice);
  }
  ...
  handleCrosshairMove(param) {
    ...
    this.startPoint ? this.updateLine(xTs, yPrice) : this.handleHoverEffect(xTs, yPrice);
  ...
  handleHoverEffect(xTs, yPrice) {
    ...
    const hoverStatus = this.isLineHovered(xTs, yPrice, linedata[0], linedata[1]);
  ...
  isLineHovered(xTs, yPrice, point1, point2) {
    ...
    const isPoint1 =
      xTs === point1.time && (Math.abs(yPrice - point1.value) * 100) / yPrice < this.hoverThreshold;
    if (isPoint1) {
      this.selectedPoint = 0;
      return true;
    }
    const isPoint2 =
      xTs === point2.time && (Math.abs(yPrice - point2.value) * 100) / yPrice < this.hoverThreshold;
    if (isPoint2) {
